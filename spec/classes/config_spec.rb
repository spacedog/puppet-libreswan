    ...
    on_supported_os.each do |os, facts|
      context "on #{os}" do
        ...
        context "libreswan::config class with parameters" do
          ...
          it { is_expected.to contain_concat_fragment('/etc/ipsec.conf_setup').with({
            'target'  => '/etc/ipsec.conf',
            'content' => "\n\nconfig setup\n  key=value\n\n",
          })
          }
          it { is_expected.to contain_concat_fragment('/etc/ipsec.conf_HEADER').with({
            'target'  => '/etc/ipsec.conf',
            'content' => '# File is managed by puppet\n',
          })
          }
          it { is_expected.to contain_concat_fragment('/etc/ipsec.conf_FOOTER').with({
            'target'  => '/etc/ipsec.conf',
